...
if ($act == 'idx') {
	...
	if ($nod == 'save') {
		...
		if ($cfPassword != $dUserLogin['password']) {
			echo 'done##', $main->toJsonData(403, 'Mật khẩu không đúng.', null); //'Username hay mật khẩu không đúng.'
		} else if ($dUserLogin['gid'] != 0) {
			echo 'done##', $main->toJsonData(403, $lang['lb_allowAdmin'], null); //'Chức năng chỉ danh cho quản trị viên.'$lang['lb_allowAdmin']
		} else {
	...
	if ($nod == "add") {
		...
		$opt->setvarname($name . '_' . $shop_id);
		$dOpt = $opt->get_by_varname();

		if (isset($dOpt['varname'])) {
			$opt->setvarname($name . '_' . $shop_id);
			$opt->setvalue($id);
			$opt->update();
		} else {
			$opt->setvarname($name . '_' . $shop_id);
			$opt->settitle($title . $dShop['name']);
			$opt->setvalue($id);
			$opt->settype(0);
			$opt->insert();
		}
		...
		$shop_id 					= $main->post('shop_id');
		$treasurer_id 				= $main->post('treasurer_id');
		$treasurer_group_id 		= $main->post('treasurer_group_id');
		$treasurer_name 			= $main->post('treasurer_name');
		$treasurer_group_name 		= $main->post('treasurer_grotreasurer_group_nameup_id');

		$opt->setvarname($treasurer_id . '_' . $shop_id);
		$dTreasurerId = $opt->get_by_varname();

		$opt->setvarname($treasurer_group_id . '_' . $shop_id);
		$dTreasurerGroupId = $opt->get_by_varname();

		if (isset($dTreasurerId['varname'])) {
			$treasurer_group->set('id', $dTreasurerId['value']);
			$treasurer = $treasurer_group->get_detail();
			if (isset($treasurer['id'])) {
				$kq['treasurer_id'] = $treasurer['id'];
				$kq['treasurer_code'] = $treasurer['code'];
				$kq['treasurer_name'] = $treasurer['name'];
			} else {
				$kq['treasurer_id'] = 0;
				$kq['treasurer_code'] = '';
				$kq['treasurer_name'] = '';
		...
		if (isset($dTreasurerGroupId['varname'])) {
			$treasurer_group->set('id', $dTreasurerGroupId['value']);
			$treasurer_gr = $treasurer_group->get_detail();
			if (isset($treasurer_gr['id'])) {
				$kq['treasurer_group_id'] = $treasurer_gr['id'];
				$kq['treasurer_group_code'] = $treasurer_gr['code'];
				$kq['treasurer_group_name'] = $treasurer_gr['name'];
			} else {
				$kq['treasurer_group_id'] = 0;
				$kq['treasurer_group_code'] = '';
				$kq['treasurer_group_name'] = '';
			}
		} else {
			$kq['treasurer_group_id'] = 0;
			$kq['treasurer_group_code'] = '';
		...
		echo 'done##', $main->toJsonData(200, 'success', $kq);
	} elseif ($nod == "auto_treasurer") {

		$treasurer_account 	= new treasurer_account();
		$treasurer_report 	= new treasurer_report();

		$is_for 			= $main->post('is_for');
		$shop_id 			= $main->post('shop_id');

		$treasurer_group->set('is_for', $is_for);
		$treasurer_group->set('shop_id', $shop_id);
		$kq 	= $treasurer_group->list_all_item('');
